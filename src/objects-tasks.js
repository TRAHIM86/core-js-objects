...
function makeWord(lettersObject) {
  // получить массив ключей и значений [ ['a', [0, 1] ], ...]
  const arr = Object.entries(lettersObject);
  const res = [];

  // для каждого значений res[index] (фактически индекс массива) =
  // ткущему ключу
  arr.forEach(([key, val]) =>
    val.forEach((index) => {
      res[index] = key;
...
function sellTickets(queue) {
  // если разница между кассой и купюрой больше 25, то false
  let sum = 0;
...
function sortCitiesArray(arr) {
  // lc возврает "порядок" сортировки (-1, 0, 1). Сортируем по
  // возрастанию по странам || если страны одинаковы (т.е. = 0),
  // то сортириуем по городам
  return arr.sort(
...
function group(array, keySelector, valueSelector) {
  ...
  // для кажого элемента получить ключ и значение
  array.forEach((elem) => {
